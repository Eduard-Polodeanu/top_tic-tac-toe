  ...
  def check_game_state
    @board_values.each_with_index do |value, index|
      if [1, 4, 7].include?(index) &&
         value == @board_values[index - 1] &&
         value == @board_values[index + 1] # horizontal win-con
        return "win", value
      end

      if [3, 4, 5].include?(index) &&
         value == @board_values[index - 3] &&
         value == @board_values[index + 3] # vertical win-con
        return "win", value
      end

      if [4].include?(index) &&
         ((value == @board_values[index - 4] && value == @board_values[index + 4]) ||
         (value == @board_values[index - 2] && value == @board_values[index + 2])) # cross win-con
        return "win", value
      end
    end
